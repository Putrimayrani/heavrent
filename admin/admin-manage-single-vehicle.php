  ...
  if(isset($_POST['upate_veh']))
    {
            ...
            $query="update tms_vehicle set v_name=?, v_reg_no=?, v_category=?, v_dpic=?, v_status=? where v_id = ?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('sssssi', $v_name, $v_reg_no, $v_category, $v_dpic, $v_status, $v_id);
            $stmt->execute();
                if($stmt)
                {
