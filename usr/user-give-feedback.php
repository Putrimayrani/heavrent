  ...
  if(isset($_POST['give_feedback']))
    {
            ...
            $query = "INSERT INTO tms_feedback (f_uname, f_content, f_status) VALUES (?, ?, ?)";
            $stmt = $mysqli->prepare($query);
            $status = "your_value_for_status"; // provide a value for f_status
            $rc = $stmt->bind_param('sss', $f_uname, $f_content, $status);
            $stmt->execute();            
                if($stmt)
                {
